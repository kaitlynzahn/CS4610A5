...
window.onload = function init() {
    ...
    //for smooth normals - use vertices and indices to calculate smooth normals, then use FlattenIndexedAttribute on the normals
    //either way, you should end up with the same number of normals as positions
    var normals = [];
    for(var i = 0; i < vertices.length; i++) {
    ...
    for(var i = 0; i < indices.length; i += 3) {
        var i1 = indices[i] - 1;
        var i2 = indices[i+1] - 1;
        var i3 = indices[i+2] - 1;
        ...
        normals[i3] = add(normals[i3], normal);
    }
    for(var i = 0; i < normals.length; i++) {
        normals[i] = normalize(normals[i]);
    }
    normals = FlattenIndexedAttribute(normals, indices);
